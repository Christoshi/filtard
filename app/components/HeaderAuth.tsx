...
export default function HeaderAuth() {
    ...
    async function loadUser() {
      ...
      if (currentUser && !currentUser.display_name) {
        if (window.location.pathname !== "/setup-username") {
          ...
          router.push(
            `/setup-username?redirect=${encodeURIComponent(currentPath)}`
          );
        }
        setLoading(false);
        return;
  ...
  // Smaller deep gold trophy (50% smaller)
  const TrophyIcon = (
    <svg width="9" height="9" viewBox="0 0 16 16" fill="#D4A017">
  ...
  if (user) {
    ...
    return (
      <div className="flex items-center gap-2 flex-shrink-0">
        <Link
          href="/leaderboard"
          className="flex items-center justify-center h-9 w-9 rounded-full hover:bg-[#D4A017]/10 transition"
          title="Leaderboard"
        >
          {TrophyIcon}
        </Link>
