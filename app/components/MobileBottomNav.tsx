    ...
    <>
      ...
      {menuOpen && (
        <div className="md:hidden fixed inset-0 z-[60]">
          ...
          {/* Sheet */}
          <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl border-t border-[#1c1f26] bg-[#0a0b0e] pb-safe">
            {/* Handle */}
            <div className="flex justify-center pt-3 pb-2">
              <div className="h-1 w-10 rounded-full bg-[#2a2e38]" />
            </div>

            {/* User info */}
            <div className="flex items-center gap-3 px-5 py-3 border-b border-[#1c1f26]">
              {user.avatar_url ? (
                <img
                  src={user.avatar_url}
                  alt=""
                ...
                <div className="h-10 w-10 rounded-full bg-[#1c1f26] flex items-center justify-center text-sm font-medium">
                  {(user.display_name || "U")[0].toUpperCase()}
                </div>
              )}
              <div>
                ...
                <p className="text-xs text-[#8b93a1]">Logged in</p>
              </div>
            </div>
            ...
            <div className="py-2">
              {(user.role === "admin" || user.role === "super_admin") && (
                <Link
                  href="/dashboard"
                  onClick={() => setMenuOpen(false)}
                  className="block px-5 py-3.5 text-[#b8ff3d] hover:bg-[#14171d] transition"
                >
                  Dashboard
                </Link>
              )}
              <Link
                href="/admin"
                onClick={() => setMenuOpen(false)}
                className="block px-5 py-3.5 text-white hover:bg-[#14171d] transition"
              >
                Submit Token
              </Link>
              <Link
              ...
              <button
                ...
                className="w-full text-left px-5 py-3.5 text-red-400 hover:bg-[#14171d] transition"
              >
                Logout
              </button>
            </div>
